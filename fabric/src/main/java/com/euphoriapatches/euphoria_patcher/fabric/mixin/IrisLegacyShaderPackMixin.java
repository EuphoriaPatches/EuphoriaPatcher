...
public class IrisLegacyShaderPackMixin {
    ...
    @Dynamic
    @Inject(
        method = "<init>(Ljava/nio/file/Path;Ljava/util/Map;Ljava/lang/Iterable;)V",
        at = @At("RETURN"),
        remap = false,
        require = 0
    )
    private void euphoriaPatcher$captureShaderPackBuildIterable(Path root, Map<String, String> changedConfigs,
                                                               @Coerce Object environmentDefines, CallbackInfo ci) {
